...
try
{
    ...
    if ($RoleGroup -eq "core")
    {
        Send-Notification "Testblob" (Get-SuccessEmoji) "Infrastructure was created for core roles in $Environment environment."
    }
    elseif ($RoleGroup -eq "meta")
    {
        Send-Notification "Testblob" (Get-SuccessEmoji) "Infrastructure was created for meta roles."
    }
