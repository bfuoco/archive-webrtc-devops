...
try
{
    ...
    foreach ($Role in $Roles)
    {
        try
        {
            ...
            {
                Send-Notification "Orbblob" (Get-SuccessEmoji) "A new AMI was created for core:$($Role):$($Environment)."
            }
            elseif ($RoleGroup -eq "meta")
            {
                Send-Notification "Orbblob" (Get-SuccessEmoji) "A new AMI was created for meta:$($Role)."
            }
        }
        catch
        {
